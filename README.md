...
|----|----------------------------|--------------------------|----------------------|-----------------|-----------------|--------------------------|
| F-001 | **Critical — Insecure Web Administration (Default Credentials & Weak Crypto)**  
HTTP service on 127.0.0.1:8080 | The device exposes a web administration interface that matches known vulnerable firmware patterns. Banner analysis and signature matching indicate use of default or bypassable credentials and outdated cryptographic components. This combination enables unauthenticated or trivially authenticated access, which is a common initial compromise vector in IoT attacks. | full_report.json → summary.top_risks[0]  
port_scan: port 8080 open  
vuln_matches: banner match  
CVEs: CVE-ACME-0001, CVE-ACME-0002 | **Critical (9.0)**  
Immediate compromise likely | 1. Start demo device on port 8080.  
2. Run scan_ports() on target.  
3. Execute match_vulns_to_scan() using known_vulnerabilities.json. | Disable all default/backdoor credentials immediately. Apply vendor firmware that removes insecure authentication and updates cryptographic libraries. If patching is unavailable, isolate the device using network segmentation and restrict administrative access to trusted hosts only. |
| F-002 | **Medium — Input Handling Weakness (Path Traversal / Reflected XSS Risk)**  
HTTP search endpoint | The HTTP interface reflects parts of user-supplied input and does not demonstrate robust sanitisation. Although exploitation confidence is low in the demo environment, the behaviour aligns with known classes of reflected XSS and path traversal flaws in embedded web servers. | vuln_matches[1]  
pentest_simulations.simulate_reflected_xss  
confidence: 0.1 (low) | **Medium (6.5)**  
Context-dependent | Run simulate_reflected_xss("http://127.0.0.1:8080/search"). Marker reflection can be observed when the endpoint echoes request paths. | Implement strict input validation and output encoding. Enforce allow-listing for file paths. Add HTTP security headers (e.g. CSP) to reduce impact even if a flaw is missed. |
| F-003 | **High — Legacy Remote Access Enabled (Telnet Exposure)**  
Configuration & Port 23 | Telnet is explicitly enabled and exposed. As Telnet transmits credentials in plaintext, it is widely exploited in IoT botnet campaigns and provides an easy interception point for attackers on the same network. | audits[] → CFG-001 / CFG-004  
open_ports: 23  
Demo config: telnet: true | **High (7.0)**  
High exploitability | Run full audit and observe Telnet in both configuration audit results and port scan output. | Disable Telnet entirely. Replace with SSH using key-based authentication where possible. Enforce firewall rules to block legacy management services from untrusted networks. |
| F-004 | **High — Administrative Interface Without Transport Encryption**  
HTTP admin access | The device serves its administrative interface over plaintext HTTP. This exposes credentials and session tokens to interception and manipulation, especially in shared or industrial networks. | audits[] → CFG-002  
admin_http: true | **High (7.0)**  
High impact | Inspect configuration audit output after running the demo device. | Enforce TLS for all administrative interfaces. Use valid certificates, enable HSTS, and apply strong session management controls. |
| F-005 | **High — Firmware Integrity Not Enforced**  
Firmware update mechanism | Firmware verification relies on hash comparison and identifies an unsigned firmware image. Without cryptographic signing and rollback protection, attackers could install malicious or downgraded firmware, compromising the device at a fundamental level. | firmware_checker → FW-001  
Unsigned firmware hash matched in firmware_hashes.json | **High (7.0)**  
Persistent compromise risk | Run configuration and firmware audit against demo firmware metadata. | Require signed firmware images and enforce signature verification during updates. Implement anti-rollback protections and authenticated update channels. |
